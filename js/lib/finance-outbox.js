/**
 * Finance outbox — queue money ops when mutate is required but offline / no cloud session.
 * Flush on reconnect; never silently drop. Permanent 4xx → mark failed (no auto-reverse).
 */
const FinanceOutbox = {
  ...
  pending() {
    return this._rows().filter(r => r && !r._deleted && (r.status === 'pending' || r.status === 'flushing'))
  },

  async enqueue({ op, idempotencyKey, payload = {} }) {
  ...
  /**
   * Flush pending outbox → Edge authorize. Safe to call often.
   */
    ...
    try {
      ...
      for (const row of batch) {
        ...
        if (res.ok || res.deduped) {
          await this._patch(row.id, { status: 'synced', lastError: '', serverResult: res.result || null })
