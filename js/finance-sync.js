...
const FinanceSync = {
  ...
  /**
   * Server-authoritative gate. A retryable network/session failure never grants
   * permission for a local money commit.
   */
  async _beforeMoneyCommit(op, payload, idempotencyKey) {
    try {
      ...
      if (typeof StudioMutateClient === 'undefined' || typeof StudioMutateClient.authorize !== 'function') {
        return { ok: false, error: 'سرویس معتبر مالی در دسترس نیست', reason: 'mutate_client_missing' }
      }
      const productionBrowser = typeof location !== 'undefined'
        && !['localhost', '127.0.0.1'].includes(String(location.hostname || ''))
      const required = productionBrowser || (typeof StudioMutateClient.requiredWhenOnline === 'function'
      ...
      if (!required && !optional) return { ok: true, developmentLocalOnly: true }
      ...
      if (res.ok) {
        ...
        let receipt = null
        if (typeof FinanceOutbox !== 'undefined' && FinanceOutbox.recordAccepted) {
          receipt = await FinanceOutbox.recordAccepted({
            op, idempotencyKey, payload: { ...payload }, serverResult: res.result || null
          })
        }
        return { ok: true, authorized: true, server: res, receipt, op, payload, idempotencyKey }
      }
      ...
      if (res.retryable || res.queueable) {
        return {
          ok: false,
  ...
  async _afterMoneyCommit(op, payload, gate) {
    ...
    if (gate?.queueOutbox && typeof FinanceOutbox !== 'undefined' && FinanceOutbox.enqueue) {
      ...
      if (gate.idempotencyKey && typeof FinanceOutbox !== 'undefined' && FinanceOutbox.markApplied) {
        await FinanceOutbox.markApplied(gate.idempotencyKey)
      }
    }
  },

  /** Apply only values returned by the authoritative ledger; never recompute them. */
  async reconcileAccepted(receipt, serverResult) {
    if (!serverResult || !Array.isArray(serverResult.balances)) {
      throw new Error('authoritative_finance_projection_missing')
    }
    for (const balance of serverResult.balances) {
      const ref = String(balance?.account_ref || '')
      if (!ref.startsWith('asset:bank:')) continue
      const bankId = ref.slice('asset:bank:'.length)
      const irr = Number(balance.balance_irr)
      if (!bankId || !Number.isSafeInteger(irr) || irr % 10 !== 0) {
        throw new Error('invalid_authoritative_bank_balance')
      }
      const bank = typeof DB.findActive === 'function'
        ? DB.findActive('banks', b => b.id === bankId)
        : DB.find('banks', b => b.id === bankId && !b._deleted)
      if (bank) await SecureDB.update('banks', bankId, { balance: irr / 10 })
    }
    const txId = receipt?.payload?.transactionId || receipt?.payload?.outTransactionId
    if (txId) {
      const tx = this._findActiveTx(txId)
      if (tx) await SecureDB.update('transactions', txId, {
        mutateStatus: 'synced',
        serverLedgerVersion: serverResult.ledgerVersion ?? null,
        serverTransactionId: serverResult.transactionId || ''
      })
    }
    if (serverResult.ledgerVersion != null) {
      const info = DB.get('studioInfo') || {}
      await SecureDB.merge('studioInfo', { ...info, ledgerVersion: serverResult.ledgerVersion })
    }
    await DB.flush?.()
    return { ok: true }
  },

  _serverAcceptedFailure(gate, error, fallback) {
    return {
      ok: false,
      error: error?.message || fallback,
      serverAccepted: !!gate?.authorized,
      needsReconciliation: !!gate?.authorized,
      idempotencyKey: gate?.idempotencyKey || ''
    }
  },
  ...
  async updateTransaction(txId, patch = {}, opts = {}) {
    ...
    const run = async () => {
      ...
      try {
        ...
        return this._serverAcceptedFailure(gate, e, 'خطا در ویرایش تراکنش — نیازمند تطبیق با سرور')
      }
    }
  ...
  async deleteTransaction(txId) {
    ...
    const run = async () => {
      ...
      try {
        ...
        return this._serverAcceptedFailure(gate, e, 'خطا در حذف تراکنش — نیازمند تطبیق با سرور')
      }
    }
  ...
  async transferBetweenBanks(opts = {}) {
    ...
    const run = async () => {
      ...
      try {
        ...
        return this._serverAcceptedFailure(gate, e, 'خطا در انتقال — نیازمند تطبیق با سرور')
      }
    }
  ...
  async recordDeposit(opts = {}) {
    ...
    try {
      ...
      return this._serverAcceptedFailure(gate, e, 'خطا در ثبت واریز — نیازمند تطبیق با سرور')
    }
  },
  ...
  async recordWithdrawal(opts = {}) {
    ...
    try {
      ...
      return this._serverAcceptedFailure(gate, e, 'خطا در ثبت برداشت — نیازمند تطبیق با سرور')
    }
  },
