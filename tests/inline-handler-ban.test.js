/**
 * Guard: Pro UI must not use inline HTML event handlers (CSP path).
 * Bans any on*= attribute in generated/template strings under studio-m/js.
 */
import { describe, it, expect } from 'vitest'
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'

const dir = path.join(path.dirname(fileURLToPath(import.meta.url)), '../studio-m/js')

// Match HTML-like inline handlers: onclick=, onkeydown=, etc. Avoid matching JS identifiers like onSave.
const INLINE_HANDLER_RE = /\s(?:on[a-z]+)\s*=\s*["']/i

describe('studio-m inline handler ban', () => {
  it('has zero inline on*="..." handlers in Pro JS sources', () => {
    const offenders = []
    for (const name of fs.readdirSync(dir).filter((f) => f.endsWith('.js'))) {
      const src = fs.readFileSync(path.join(dir, name), 'utf8')
      const lines = src.split('\n')
      lines.forEach((line, i) => {
        if (INLINE_HANDLER_RE.test(line)) {
          offenders.push(`${name}:${i + 1}: ${line.trim().slice(0, 120)}`)
        }
      })
    }
    expect(offenders).toEqual([])
  })
})
