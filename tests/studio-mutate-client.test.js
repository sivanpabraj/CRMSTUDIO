...
describe('StudioMutateClient SaaS authority', () => {
  ...
  beforeEach(() => {
    ...
    prev.locationDescriptor = Object.getOwnPropertyDescriptor(globalThis, 'location')
  ...
  afterEach(() => {
    ...
    if (prev.locationDescriptor) Object.defineProperty(globalThis, 'location', prev.locationDescriptor)
  ...
  it('cannot disable server finance authority with production browser flags or local settings', () => {
    Object.defineProperty(globalThis, 'location', {
      configurable: true,
      value: { hostname: 'crm.example.ir' }
    })
    globalThis.__SM_MUTATE_REQUIRED = false
    globalThis.DB = { get: () => ({ mutateRequiredWhenOnline: false, cloudEnabled: false }) }
    expect(StudioMutateClient.requiredWhenOnline()).toBe(true)
  })

  it('fail-closed when required and offline is retryable without local commit authority', async () => {
    ...
    expect(res.queueable).toBeUndefined()
    expect(res.reason).toBe('offline_blocked')
  })

  it('no cloud session is retryable but never authorizes a local money write', async () => {
    globalThis.__SM_MUTATE_REQUIRED = true
  ...
  it('uses ledger version zero for a new authoritative ledger', async () => {
    globalThis.__SM_MUTATE_REQUIRED = true
    globalThis.Cloud = {
      resolvedConfig: () => ({ url: 'https://abc.supabase.co' }),
      client: async () => ({ auth: { getSession: async () => ({ data: { session: { access_token: 'tok' } } }) } })
    }
    globalThis.fetch = vi.fn(async () => ({ ok: true, json: async () => ({ ok: true, result: { ledgerVersion: 1 } }) }))
    const res = await StudioMutateClient.authorize('record_deposit', {
      transactionId: 'new-ledger-tx', bankId: 'bank-1', amount: 1
    })
    expect(res.ok).toBe(true)
    expect(JSON.parse(globalThis.fetch.mock.calls[0][1].body).expectedVersion).toBe(0)
  })

  it('rejects an invalid local ledger version before sending a command', async () => {
    globalThis.__SM_MUTATE_REQUIRED = true
    globalThis.DB = { get: () => ({ supabaseStudioId: 'studio_1', ledgerVersion: -1 }) }
    globalThis.fetch = vi.fn()
    const res = await StudioMutateClient.authorize('record_deposit', {
      transactionId: 'invalid-version', bankId: 'bank-1', amount: 1
    })
    expect(res).toMatchObject({ ok: false, reason: 'invalid_expected_version' })
