...
describe('Cloud Google OAuth helpers', () => {
  ...
  it('stashOAuthConfig keeps only navigation intent and removes browser endpoint overrides', async () => {
    const { default: Cloud } = await import('../js/cloud.js')
    sessionStorage.setItem('sm_cloud_url', 'https://attacker.supabase.co')
    sessionStorage.setItem('sm_cloud_key', 'sb_publishable_attacker')
    Cloud.stashOAuthConfig({ intent: 'signin' })
    expect(sessionStorage.getItem('sm_cloud_url')).toBeNull()
    expect(sessionStorage.getItem('sm_cloud_key')).toBeNull()
    expect(sessionStorage.getItem('sm_oauth_intent')).toBe('signin')
    expect(sessionStorage.getItem('sm_oauth_return')).toBe('/studio-m/index.html#settings')
  })
