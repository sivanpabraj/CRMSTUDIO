...
test.describe('Finance auth e2e', () => {
  ...
  test('password login → accounting → deposit persists locally', async ({ page }) => {
    ...
    test.skip(optionCount < 2, 'No bank accounts seeded — seed a bank before finance e2e')
    ...
    const durable = await page.evaluate(() => {
      try {
        const raw = localStorage.getItem('studio_db_v5')
        if (!raw) return { ok: false, reason: 'no_db' }
        const db = JSON.parse(raw)
        const txs = (db.transactions || []).filter(t => !t._deleted && Number(t.amount) === 125000)
