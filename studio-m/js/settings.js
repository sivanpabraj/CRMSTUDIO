...
const SMSettings = {
  ...
  _profileTab() {
    ...
    const isDefaultPhone = !!user?.isBootstrapAdmin
  ...
  _widgetsTab() {
    ...
    return `<div class="sm-card sm-settings-full">
      <div class="sm-card-head">
        ...
        <button type="button" class="sm-btn sm-btn-sm sm-btn-primary" ${SMEvents.attrs('SM.navigate', ['dashboard'])}><i class="fas fa-eye"></i> مشاهده پیشخوان</button>
      </div>
      <div class="sm-card-body">
  ...
  _smsTab(info) {
    ...
    return `<div class="sm-card"><div class="sm-card-head"><div class="sm-card-title">تنظیمات پیامک (SMS)</div>
      ...
      <div class="sm-card-body">
        <p style="font-size:.85rem;color:var(--sm-text-muted);margin-bottom:14px">ارائه‌دهنده و کلید پیامک فقط از طریق secretهای Edge Function مدیریت می‌شوند و در مرورگر قابل مشاهده یا تغییر نیستند.</p>
        ${SMUI.badge(configured ? 'Edge Function متصل' : 'اتصال ابری ناقص', configured ? 'success' : 'warning')}
      ...
      <div class="sm-card">
        ...
        <div class="sm-card-body">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
            ...
            <button type="button" class="sm-btn sm-btn-ghost" ${SMEvents.attrs('SMSettings.cloudSignInGoogle')}><i class="fas fa-at"></i> ورود با Google</button>
  ...
  async saveSms() {
    ...
    await SecureDB.merge('studioInfo', settings)
    SM.toast('تنظیم یادآوری پیامک ذخیره شد', 'success')
    SM.navigate('settings')
  },
