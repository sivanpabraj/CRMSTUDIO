...
این تابع فقط برای مدیر فعال همان استودیو یک نسخهٔ بازیابی تغییرناپذیر می‌سازد. `studio_id` به‌تنهایی قابل اعتماد نیست و عضویت مدیر با `auth.uid()` داخل تابع دوباره کنترل می‌شود. payload باید شیء JSON، checksum باید SHA-256 و اندازه حداکثر ۲۰ مگابایت باشد. جدول مقصد برای `anon` و نوشتن مستقیم `authenticated` بسته است؛ فقط خود RPC اجازهٔ درج دارد. بکاپ هیچ‌گاه منبع احراز هویت یا مجوز ورود نیست.
