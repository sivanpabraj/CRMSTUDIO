-- Reject unsupported trigger attachments before resolving table-specific NEW
-- fields. This preserves fail-closed behavior even when the attached relation
-- does not expose contract_id/studio_id.
...
begin
  if tg_table_name not in (
    'erp_contract_revision_requests',
